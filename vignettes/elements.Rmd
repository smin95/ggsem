---
title: "1. Drawing Elements (In Progress)"
---
...
To begin with, let's run the app locally using the code below (faster than online: https://smin95.shinyapps.io/ggsem_shiny/):
...
p1 <- csv_to_ggplot(
  ...
  vertical_position = 0
) # Default verfical position on the app
```
...
We can also chip away the extra space by directly dealing with the axis limits of the figure. First, we extract the current range of the y-axis limit of the `p2` object using the function `get_axis_range()`, which only has one argument (a ggplot2 object).
