...
Next, we can set the gradient filling color of the points by setting the inputs as:
...
Now, we have a small navy-colored network of eleven points.
...
There are many ways in which the poinst can be connected with lines (edges) using the **ggsem** app. Let's shift our attention to the dropdown menu of **Choose Edge Connection Type** under **Layout  Functions**. Ther are several options: 
...
1. **Line Color for Auto-Generated Edges**: #CACFDB

2. **Line Width for Auto-Generated Edges**: 2

3. **Line Alpha for Auto-Generated Edges**: 0.5

```{r,fig.align = 'left', echo=F, out.width = '40%'}
knitr::include_graphics("fig2d.png")
```

Next, we will create another network using these layout functions. But before that, we will need to **lock** these points, so that later usages of layout functions will not affect their positions and edge connections.

To do so, we go back to the **Point** menu, and click the **Lock Points** button (blue rectangle) under **Layout Functions**. 
...
1. **Line Color for Auto-Generated Edges**: #DBCAD3

2. **Line Width for Auto-Generated Edges**: 1

3. **Line Alpha for Auto-Generated Edges**: 0.3
...
p3 <- csv_to_ggplot(points_data = points_data,
...
First, we extract the plot's ranges of x-axis and y-axis. 
...
p4 <- p3 + annotate('text', label = 'First Network', x = 0, y = 16, 
              fontface='bold', size = 8) +
  annotate('text', label = 'Second Network', x = 29, y = 16, 
           fontface='bold', size = 8)
```
...
```{r, message = F, warning = F}
net1 <- csv_to_ggplot(points_data = points_data1,
...
net2 <- csv_to_ggplot(points_data = points_data2,
...
We can adjust the visibility of the text on each node by applying unique color and adjusting its text size. To use custom colors in `geom_text()`, `scale_color_identity()` should also be used to ensure that the colors are properly rendered. 

```{r, fig.width = 6, fig.height = 6}
cList <- c('#494949', '#494949', '#5B5B5B', '#5B5B5B', '#EBEBEB',
           '#EBEBEB', '#EBEBEB', '#EBEBEB', '#EBEBEB', '#EBEBEB',
           '#FDFDFD') # text color for each node label

texts_data$color <- cList

net1 + 
  geom_text(aes(x = x, y = y, label = label, color = color), data = texts_data, 
