...
Next, we will create another network using these layout functions. But before that, we will need to **lock** these points, so that layout functions will not affect their positions and edge connections.
...
We can adjust the visibility of the texts by applying unique colors to each node and adjusting its text size. To use custom colors in `geom_text()`, `scale_color_identity()` should also be used to ensure that the colors are properly rendered. 
