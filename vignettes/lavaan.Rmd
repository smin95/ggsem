...
Both `~` and `=~` are shown as a one-way arrow in SEM diagrams, and these are recognized by the **ggsem* app.
...
After setting the inputs as specified, we can click the **Draw Graph from Lavaan** button, which creates new graphical outputs. **Apply Changes** button (in the blue box) modifies the aesthetics of the existing graphical outputs that are created from the **Lavaan Input** menu. So, if you wish to change the layout (from Sugiyama to Kamada-Kawai) while keep all other aesthetics the same, then you can simply change the layout in the **Choose Layout Algorithms** to *Kamada-Kawai*, and then click the **Apply Changes** button.

It is important to note that **Apply Changes** button modify aesthetics of graphical elements that have been created from the **Lavaan Input** menu but not from others. For example, if users create self-loop arrows later, these arrows will not be affected by the **Apply Changes** button from the **Lavaan Input** menu because these have not been produced together from the **lavaan** strings. To identify which components have been generated through **lavaan**, check the *lavaan* column in the output tables; if it is set to *TRUE*, then it is part of the *lavaan* output, and it is susceptible to the aesthetic changes brought by the **Apply Changes** button. For self-loop arrows, this column will be *FALSE*.  

Readers can also explore with the rest of layout choices to see how layout affects the SEM diagram. Then, they can the CSV outputs for points, lines and text annotations. Also, if you are not satisfied with the current look of the SEM, you can also change numerical inputs in the table **directly**, such as the X and Y coordinates of the points, lines and texts. 
...
sem_plot <- csv_to_ggplot(points_data = points_data,
                    lines_data = lines_data,
                    annotations_data = texts_data,
                    element_order = c('lines', 'points', 'annotations')) # order priority: lines < points < texts 
```
...
As in the two previous chapters, we find empty space, which can be removed by reducing the ranges of x- and y-axes. First, we extract the x and y ranges.

```{r}
plot_build <- ggplot_build(sem_plot)
...
sem_plot2b <- sem_plot2 +
  annotate('text', label = 'Intelligence Measure (SEM)', x = 4, y = 14, size = 7, 
           fontface = 'bold')
```
...
sem_plot3 <- csv_to_ggplot(points_data = points_data,
                    lines_data = lines_data,
                    annotations_data = texts_data,
                    element_order = c('lines', 'points', 'annotations')) # order priority: lines < points < texts 
```
...
```{r}
sem_plot4b <- sem_plot4 +
  annotate('text', label = 'Intelligence Measure (SEM)', x = 4, y = 14, size = 7, 
           fontface = 'bold')
```

```{r}
ggsave('sem_plot4b.png',sem_plot4b, width = 7.5, height = 5)
```

```{r,fig.align = 'left', echo=F, out.width = '90%'}
knitr::include_graphics("sem_plot4b.png")
```

Unfortunately, the size of the latent variable is a bit too small. We can increase it by modifying the CSV file of 
`points_data`. 

**ggsem** automatically assigns circle to latent variables and square to observed variables, as this is the standard in SEM diagrams. So, we identify the rows in the data frame with circle outputs, and then modify their size.
...
sem_plot5 <- csv_to_ggplot(points_data = points_data,
                    lines_data = lines_data,
                    annotations_data = texts_data,
...
Also, in our specific example, the arrows do not end exactly at the same y-coordinate at one end (`y_end`). This is because some lines are vertically oriented while some others are diagonally oriented. We can fix this too by changing the values in `y_end` column in `lines_data`.

```{r}
head(lines_data$y_end) # print first six values

lines_data$y_end <- -6.15 # all values are same
```

Then, we can recreate the figure with `csv_to_ggplot()`. 

```{r, warning = F, message = F}
sem_plot6 <- csv_to_ggplot(points_data = points_data,
                    lines_data = lines_data,
                    annotations_data = texts_data,
                    element_order = c('lines', 'points', 'annotations')) +  
  coord_cartesian(ylim = c(-18,18), xlim = c(-16,24)) +
...
```{r}
ggsave('sem_plot6.png',sem_plot6, width = 7.5, height = 5)
```

```{r,fig.align = 'left', echo=F, out.width = '90%'}
knitr::include_graphics("sem_plot6.png")
```
