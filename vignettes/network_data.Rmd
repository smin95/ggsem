...
When saving as a *png* file, users can select the x range and y range of the figure to remove surrounding white space. However, in this example, we will remove it in script-based workflow.

```{r,fig.align = 'left', echo=F, out.width = '40%', fig.align='center'}
knitr::include_graphics("save_viz.png")
```

This is the final output, saved as png image from the *ggsem* application, without removing any white space from the plotting space.
